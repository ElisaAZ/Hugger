...
properties {
	...
	$testResultsDir = "$projectRoot/Hugger.Tests/TestResults"
	$version = "1.0.0-ci-local"
	$owner = 'Dane Schilling'
	$yearInitiated = '2020'
}
...
task Run-Tests -depends Run-Restore, Build-Solution -description "Run unit tests" {
	exec { dotnet test --configuration $configuration  --logger "trx;LogFileName=Testresults.xml" } -workingDirectory Hugger.Tests
}
...
task Package-Local -description "Build local nuget file" {
	exec { dotnet pack -c Release -o $buildDir --include-symbols --no-build } -workingDirectory $projectRoot/Hugger
}
